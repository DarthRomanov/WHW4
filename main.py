...
class HttpHandler(BaseHTTPRequestHandler):
    ...
    def send_html_file(self, filename, status=200):import urllib.parse
import mimetypes
import pathlib
import socket
import json
...
def send_data_to_socket(data):  
    ...
    client_socket.sendto(data.encode(), ('127.0.0.1', 5000))
    print(f"Send data to socket {data}")
    client_socket.close()
...
def socket_udp():
    ...
    try:
        while True:
            ...
            print(f"Receive data from socket {message_data}")
            data_dict = {key: value for key, value in [el.split('=') for el in message_data.split('&')]}
            save_message_json(data_dict, current_time)
...
def save_message_json(message_data, current_time):
    ...
    print(f"Save data to file {json_data}")
